...
written to ./live_state.json so it can be round-tripped through storage.
    ...
    python evotrader_bundle.py holdout-margin-audit  # of the real recorded holdout rejections, how many were only margin failures (raw beat the champion)?
    ...
    python evotrader_bundle.py disagreement-sweep  # keep_frac sweep: does the raw-fitness-vs-excess-return disagreement rate hold up across calendar windows?
    python evotrader_bundle.py short-headroom   # how much real upside is on the table from shorting, before wiring any short-opening signal?
"""
...
def _reconstruct_champion_genome(version, lineage):
    ...
    no disk I/O), never `Genome.champion()`. `champion()` reads whatever
    `state/genomes/champion.json` currently holds, and the `evolve` command
    unconditionally overwrites that file with the *live* champion at the
    start of every run (`g0.save("champion")`, so it can diff before/after
    to detect a real promotion) -- so any reconstruction diagnostic run in
    the same container after an `evolve` call was picking up the live
    champion's genes as its "version 1 seed" and silently mis-reconstructing
    every version, including the ones with real patches recorded (a patch
    only overwrites the specific genes it names; anything else still came
    from this wrong base). Found 2026-09-06 when `succession-audit` reported
    identical numbers for v1/v2/v3 after a same-session `evolve` run had
    already overwritten champion.json."""
    from core.genome import Genome
...
def main():
    ...
    if cmd == "tick":
        ...
        if mem.get("champion_version") == g0.version:
            # migrate_tested_entry() normalizes both old-format entries (a
            # full gene patch, persisted before AGENTS.md item 12's fix) and
            # new-format ones (already a hash) to the same hash identity, so
            # this reads a mixed list correctly and the write below persists
            # only hashes from here on.
        ...
        run = EvolutionRun(data, champion=g0, seed=None, initial_tested=init_tested,
        ...
        for label, a, b in windows:
            ...
            print(f"  {label:<10s} {str(replay.index[start])[:10]:<12s} "
                  ...
                  f"{bench['max_dd']:>7.1%}")
    elif cmd == "short-headroom":
        # Diagnostic: AGENTS.md item 5 (short selling) shipped Phase 1
        # broker mechanics (PaperBroker.short()/.cover(), 2026-09-08) and
        # the discretionary cover-intent wiring (2026-09-14), but nothing in
        # the live path has ever proposed OPENING a short -- that half is
        # explicitly an unscoped owner decision (whether/how the Researcher
        # should be allowed to). Before that gets designed, this answers the
        # question a real proposal would need first: is there even real
        # theoretical upside on the table? Same read-only, genome-independent
        # discipline as `regime` (reuses its exact fold/holdout windows and
        # universe/interval/cash), but reports `loop.engine.
        # benchmark_sell_short` next to `benchmark_buy_hold` for each one --
        # a static, un-managed, equal-weight short of the same basket, using
        # the real PaperBroker.short()/.mark()/.cover() mechanics (so fees
        # and slippage are the genome's own broker.fee_bps/slippage_bps, not
        # invented), at two modelled per-bar borrow rates: 0.0 (isolates
        # fee+slippage drag alone) and `--borrow-bps-per-bar` (default 3.0,
        # a stated modelling assumption in the same family as fee_bps/
        # slippage_bps, NOT a measured real crypto borrow rate -- nothing in
        # this repo has ever measured one). "captured" shows how much of the
        # zero-cost theoretical edge (`-buy&hold return`, only meaningful in
        # a window buy&hold actually lost) survives real costs in the
        # higher-borrow scenario. No genome mutation, no council, no
        # live_state.json touch -- never more than a reason to consider
        # designing the real (owner-gated) wiring, never itself that design.
        from constitution import HOLDOUT_FRAC, N_FOLDS
        from core import market
        from core.market import Replay
        from loop.engine import benchmark_buy_hold, benchmark_sell_short
        interval = None
        if "--interval" in sys.argv:
            interval = sys.argv[sys.argv.index("--interval") + 1]
        borrow_bps = 3.0
        if "--borrow-bps-per-bar" in sys.argv:
            borrow_bps = float(sys.argv[sys.argv.index("--borrow-bps-per-bar") + 1])
        g0 = acct.genome
        interval = interval or g0.bar_interval
        data = market.load_universe(g0.universe, interval, 4.0)
        if not data:
        ...
        slippage_bps = float(bcfg.get("slippage_bps", 5.0))
        bpy = market.BARS_PER_YEAR.get(interval, 365.25)
        print(f"[short-headroom] {len(data)} symbols, {interval} bars, {n} total "
              f"bars, fee={fee_bps:.0f}bps slippage={slippage_bps:.0f}bps "
              f"borrow={borrow_bps:.1f}bps/bar (modelled assumption, not measured)",
              flush=True)
        print()
        print("SHORT-THE-INDEX HEADROOM BY FOLD/HOLDOUT -- equal-weight, static, "
              "un-managed (no signal, no exit discipline)")
        print("=" * 108)
        print(f"  {'window':<10s} {'bars':>6s} {'buy&hold':>10s} {'short 0bp':>10s} "
              f"{'short+borrow':>13s} {'captured':>10s}")
        for label, a, b in windows:
            start = max(60, int(n * a))
            end = min(n - 1, int(n * b))
            if end - start < 5:
                print(f"  {label:<10s} window too short ({end - start} bars)")
                continue
            bh = benchmark_buy_hold(replay, g0.universe, start, end, cash, bars_per_year=bpy)
            s0 = benchmark_sell_short(replay, g0.universe, start, end, cash,
                                      fee_bps=fee_bps, slippage_bps=slippage_bps,
                                      borrow_bps_per_bar=0.0, bars_per_year=bpy)
            sb = benchmark_sell_short(replay, g0.universe, start, end, cash,
                                      fee_bps=fee_bps, slippage_bps=slippage_bps,
                                      borrow_bps_per_bar=borrow_bps, bars_per_year=bpy)
            if not (bh and s0 and sb):
                print(f"  {label:<10s} no benchmark data")
                continue
            headroom = -bh["total_return"]
            captured = f"{sb['total_return'] / headroom:>+9.0%}" if headroom > 0 else "n/a (bull)"
            print(f"  {label:<10s} {end - start:>6d} {bh['total_return']:>+9.1%} "
                  f"{s0['total_return']:>+9.1%} {sb['total_return']:>+12.1%} "
                  f"{captured:>10s}")
        print()
        print("buy&hold: equal-weight long, zero cost (loop.engine.benchmark_buy_hold). "
              "short 0bp: equal-weight short at the genome's own fee/slippage, no "
              "borrow cost. short+borrow: same, plus the modelled borrow rate above. "
              "captured: short+borrow's return as a fraction of the zero-cost "
              "theoretical short edge (-buy&hold), only meaningful where buy&hold "
              "itself lost money -- a bull window has no bear edge for a short to "
              "capture, hence 'n/a (bull)'. Read-only throughout -- no genome, "
              "council, or live_state.json touch. Does not decide whether to build "
              "real short-opening signals; see AGENTS.md item 5 for what this first "
              "result does and doesn't justify.")
        ...
        # Diagnostic: the 2026-08-28 16:32 UTC guardian-weighted shadow-evolve
        # session ran a real 25-generation search and found 16 of 69
        # holdout-tested candidates (23%) beat champion v3's own sealed-
        # holdout score in raw terms, yet every one was still rejected
        # because holdout_accepts() requires beating champion + margin, not
        # just beating champion -- see AGENTS.md item 4's "Next" for the
        # design tension this exposed (an unusually lucky champion holdout
        # draw can entrench indefinitely). This answers the same question
        # against `live_state.json`'s own recorded lineage instead of a
        # fresh shadow search: no new backtest, no market data, reads
        # `acct.lineage` only, same "never touches live_state.json"
        # guarantee as `holdout-pressure`. `raw_holdout_beats()` is a lower-
        # bound diagnostic, not a proposed replacement rule -- see its
        # docstring for why a raw beat ignoring the multiple-testing margin
        # is not the same thing as "should have been promoted."
        ...
        print()
        cols = ["version", "fold-agg fit", "dd-corr fit", "trust-cont fit",
                "full-hist maxDD", "full-hist fit", "hard-fail?", "excess ret"]
        widths = [10, 13, 12, 16, 16, 14, 11, 11]
        ...
        for v in versions:
            ...
            trust_cont_stats = dd_trust_continuous_stats(ev, g, res["stats"])
            trust_cont_fit = _fitness_fn(trust_cont_stats)
        ...
        print("trust-cont fit: fitness() under dd_trust_continuous_stats() instead -- "
              "a diagnostic-only, two-sided alternative that always trusts the "
              "continuous replay's max_dd rather than min(fold-merged, continuous). "
              "NOT what accepts() gates on; shows what the verdict would look like if "
              "the still-open one-directional min()-only blind spot (fold-merged can "
              "OVERSTATE true risk via fold rebasing, not just understate it -- see "
              "v2 below) were ever corrected both ways. A real change to the gate's "
              "policy is a design decision, not made here.")
        ...
        # v1's base MUST be Genome() (hardcoded SEED_GENOME, no disk I/O), never
        # Genome.champion() -- see _reconstruct_champion_genome's own docstring
        # for why: champion.json is shared, unsandboxed disk state that any
        # same-container `evolve` call (or, found 2026-09-07, a concurrently
        # running test) can overwrite with the wrong genome first.
        genome_cache = {1: Genome()}
    ...
    elif cmd == "exit-gene-test":
        # Diagnostic: the "let search decide" step exit-role-test's own
        # docstring named as the next move (2026-08-27 18:54/21:48 UTC
        # entries; see AGENTS.md's Current state / Next steps item on
        # consult_moderate's exit-role asymmetry). exit-role-test measured
        # a MONKEYPATCH (sell intents stripped in Python) on one full-history
        # replay -- informative but not something the real promotion
        # pipeline could ever accept, since nothing was proposed as an
        # actual genome patch or run through the acceptance gates. This
        # builds the real gene patch(es) -- consult_moderate's own
        # `exit_trend_below`/`exit_rsi` -- as genuine Genome.child() diffs,
        # then runs each through the SAME machinery EvolutionRun.generation()
        # uses for a real top-3 candidate: Evaluator.evaluate() for
        # fold-aggregate fitness, dd_corrected_stats() (the 2026-08-22
        # fold-dd-blindspot fix) before constitution.accepts(), and --
        # only for whichever candidate clears that gate, in ranked order,
        # matching generation()'s own sequential short-circuit -- the sealed
        # holdout via constitution.holdout_accepts(). Both gates use this
        # champion's REAL cumulative researcher_memory counts (tested
        # candidates, holdout draws) from live_state.json, so the
        # multiple-testing margin is the one a real candidate would actually
        # face, not an artificially easy n=1. Two hand-designed candidates,
        # not a blind Researcher-driven search -- cheaper than a real
        # `evolve` invocation because nothing is proposed or mutated, only
        # the two hypotheses the Next-steps entry already named are tried.
        # Read-only: never calls Genome.save()/promote(), never writes
        # lineage.jsonl or live_state.json. Composes only already-tested
        # Evaluator.evaluate/dd_corrected_stats/Genome.child plus
        # constitution.accepts/holdout_accepts, same precedent as every
        # other diagnostic in this file.
        #
        # `--also-version N` reconstructs a past champion the same way every
        # other champion-reconstruction diagnostic here does
        # (_reconstruct_champion_genome, from AGENTS.md item on exit-gene-test's
        # 2026-08-28 00:56/04:04 UTC entries: "unwired ... researcher_memory
        # lookup only supports the live champion"). live_state.json's
        # researcher_memory only ever holds cumulative counts for whichever
        # version is champion *right now* (reset on every promotion, see
        # `mem.get("champion_version") == g0.version` below), so a
        # reconstructed past champion always takes the n_candidates=0 /
        # holdout_draws=0 branch -- the same fallback this command already
        # used whenever mem's champion_version didn't match. That understates
        # a real historical margin (a past champion in its own day would have
        # faced whatever it had actually accumulated by then), so treat a
        # reconstructed champion's verdict as an optimistic upper bound on
        # whether these two exit-gene variants could ever clear its gate, not
        # an exact replay of history.
        ...
        variants = [
            ...
            ("narrower exit (harder to trigger)",
             [("agents.consult_moderate.genes.exit_trend_below", -0.05),
              ("agents.consult_moderate.genes.exit_rsi", 90.0)]),
        ]
        ...
        for champ_label, g in champions:
            ...
            cur = g.genes("consult_moderate")
            print(f"[exit-gene-test] champion {champ_label} ({len(data)} symbols), "
                  f"consult_moderate exit genes now: "
                  f"exit_trend_below={cur.get('exit_trend_below', 0.0)}, "
                  f"exit_rsi={cur.get('exit_rsi', 80)}", flush=True)
            ...
            # Both candidates are one proposal batch, exactly like the
            # mutations EvolutionRun.generation() proposes together in step 2
            # -- they share one n_candidates for the fold-gate margin, not an
            # incrementing count per candidate (generation() computes
            # n_tested once, before its loop).
            ...
            for label, patch in variants:
                child = g.child(patch, note=f"exit-gene-test: {label}")
                ...
                rows.append((label, fold_fit, fold_ok, fold_why, ho_fit, ho_ok,
                            ho_why, verdict))

            cols = ["variant", "fold-agg fit", "fold gate", "holdout fit",
                    "holdout gate", "verdict"]
            widths = [32, 12, 36, 12, 44, 14]
            print()
            print("".join(c.rjust(w) for c, w in zip(cols, widths)))
            print("-" * sum(widths))
            for label, fold_fit, fold_ok, fold_why, ho_fit, ho_ok, ho_why, verdict in rows:
                row = [
                    label,
                    f"{fold_fit:.3f}" if math.isfinite(fold_fit) else "-inf",
                    ("OK: " if fold_ok else "NO: ") + fold_why[:32],
                    "-" if ho_fit is None else (
                        f"{ho_fit:.3f}" if math.isfinite(ho_fit) else "-inf"),
                    ("OK: " if ho_ok else "NO: ") + ho_why[:40] if ho_fit is not None
                    else ho_why[:40],
                    verdict,
                ]
                print("".join(c.rjust(w) for c, w in zip(row, widths)))
            ...
            print(f"champion {champ_label}: fold-agg fitness {champ_fit:.3f}, "
                  f"sealed-holdout fitness "
                  f"{champ_ho_fit if math.isfinite(champ_ho_fit) else float('-inf'):.3f}.")

        print("fold gate: constitution.accepts() on dd-corrected merged stats -- "
              "the identical function and dd_corrected_stats() fix "
              "EvolutionRun.generation() applies to a real top-3 candidate. "
              "holdout gate: constitution.holdout_accepts() against the sealed "
              "holdout, only reached (in ranked order, stopping at the first "
              "acceptance) for whichever candidate(s) clear the fold gate -- "
              "both using the champion's real researcher_memory counts where "
              "available (only ever populated for whichever version is "
              "champion right now; a --also-version reconstruction always "
              "uses 0 tested / 0 holdout draws, an optimistic upper bound, "
              "not this account's real history for that version) so the "
              "multiple-testing margin matches what a real candidate would "
              "face. Never persisted -- no genome file, lineage.jsonl, or "
              "live_state.json write. 'WOULD PROMOTE' means what the real "
              "gate would have said about this exact patch, not that "
    ...
    elif cmd == "disagreement-sweep":
        # Diagnostic: formalises the keep_frac sweep the 2026-08-29 19:12 UTC
        # session's own "Next" section flagged as the natural follow-up to
        # itself and the 16:28 UTC session -- both investigated whether raw
        # ranking_fitness and excess-over-benchmark return disagree about
        # which of champion/challenger is better, but as one-off throwaway
        # sandbox scripts (deleted after extracting results), one truncation
        # point checked per session. loop.evolve.disagreement_scan (new,
        # tested: tests/test_disagreement_scan.py) mirrors the same
        # generation()-gating logic those scripts used by hand, as a
        # reusable function instead of a fifth throwaway script. This
        # command runs it at each --keep-fracs point: truncates every
        # symbol's loaded history to its first keep_frac of bars before
        # handing it to a fresh Evaluator, shifting the existing 85/15
        # fold/holdout split onto an earlier slice of the same real data
        # (same trick the 19:12 UTC session used by hand) -- keep_frac=1.0
        # is today's actual current window.
        #
        # Read-only, same guarantees as every other diagnostic here:
        # disagreement_scan's own contract is to never call
        # Genome.save()/.promote()/EvolutionRun._record() -- no
        # live_state.json write, no genome file, no lineage.jsonl write.
        from core import market
        from loop.evolve import Evaluator, disagreement_scan
        from agents.researcher import Researcher

        keep_fracs = [1.0]
        if "--keep-fracs" in sys.argv:
            keep_fracs = [float(x) for x in
                         sys.argv[sys.argv.index("--keep-fracs") + 1].split(",")]
        generations = 15
        if "--generations" in sys.argv:
            generations = int(sys.argv[sys.argv.index("--generations") + 1])
        n_blind = 14
        if "--n-blind" in sys.argv:
            n_blind = int(sys.argv[sys.argv.index("--n-blind") + 1])
        seed_live = "--fresh" not in sys.argv

        g0 = acct.genome
        print(f"[disagreement-sweep] loading {len(g0.universe)}-symbol universe "
              f"({g0.bar_interval} bars) ...", flush=True)
        full_data = market.load_universe(g0.universe, g0.bar_interval, 4.0)
        if not full_data:
            print("no market data")
            sys.exit(1)

        mem = acct.researcher_memory or {}
        if seed_live and mem.get("champion_version") == g0.version:
            init_tested = {Researcher.migrate_tested_entry(item) for item in mem.get("tested", [])}
            init_stagnation = int(mem.get("stagnation", 0))
            init_holdout_draws = int(mem.get("holdout_draws", 0))
        else:
            init_tested, init_stagnation, init_holdout_draws = set(), 0, 0

        print(f"[disagreement-sweep] champion v{g0.version}, {generations} generation(s) "
              f"per point, n_blind={n_blind}, seeded from "
              f"{'live researcher_memory' if seed_live else 'a blank slate'} "
              f"(tested={len(init_tested)}, stagnation={init_stagnation}, "
              f"holdout_draws={init_holdout_draws})")
        print()
        print(f"  {'keep_frac':>9s} {'champ fold-fit':>15s} {'fold n':>7s} "
              f"{'fold dis%':>9s} {'risky':>6s} {'cons':>5s}  "
              f"{'ho n':>5s} {'ho dis%':>8s} {'risky':>6s} {'cons':>5s}")
        for kf in keep_fracs:
            data = {s: df.iloc[:max(1, int(len(df) * kf))] for s, df in full_data.items()}
            ev = Evaluator(data)
            champ_fit = ev.evaluate(g0)["aggregate_fitness"]
            researcher = Researcher(seed=None)
            result = disagreement_scan(
                g0, ev, researcher, generations=generations, n_blind=n_blind,
                initial_tested=set(init_tested), initial_stagnation=init_stagnation,
                initial_holdout_draws=init_holdout_draws)
            fs, hs = result["fold_stage"], result["holdout_stage"]

            def _pct(x):
                return "n/a" if x is None else f"{x:.1%}"
            promo_note = (f"  ({result['shadow_promotions']} shadow promotion(s), "
                          f"never persisted)" if result["shadow_promotions"] else "")
            print(f"  {kf:>9.2f} {champ_fit:>15.3f} {fs['n']:>7d} "
                  f"{_pct(fs['disagreement_rate']):>9s} {fs['risky']:>6d} "
                  f"{fs['conservative']:>5d}  {hs['n']:>5d} "
                  f"{_pct(hs['disagreement_rate']):>8s} {hs['risky']:>6d} "
                  f"{hs['conservative']:>5d}{promo_note}")
        print()
        print("Reading it: keep_frac 1.0 is today's real fold/holdout window; smaller "
              "values shift the same 85/15 split onto an earlier slice of the same real "
              "history, testing whether a disagreement rate holds up across calendar "
              "windows or is itself window-dependent (as-of-drift). Never touches "
              "live_state.json, never promotes anything -- every champion here is g0 held "
              "in memory only, disagreement_scan's own contract. Does not touch the "
              "still-open 'should the selection metric be redefined' question, which "
              "stays the owner's call.")
    elif cmd == "boldness-scan":
        # Diagnostic: agents.researcher.Researcher.perturb's own docstring
        # already works out that jump_p (fresh-uniform-redraw chance per
        # mutated gene) saturates by boldness ~4.6 and genes_per (how many
        # genes one proposal touches) saturates at every gene in GENE_SPACE
        # by boldness ~82 -- past that, a stagnation counter that keeps
        # climbing (the live champion's researcher_memory["stagnation"] has
        # been in the hundreds-to-thousands for a long stretch, see
        # AGENTS.md "Current state") changes nothing about what perturb
        # actually proposes: every non-exploitative candidate is already a
        # full-genome, mostly-uniform-random redraw. That has been a
        # hypothesis read off the arithmetic, never a measurement. This
        # command is the measurement: loop.evolve.boldness_scan (new,
        # tested: tests/test_boldness_scan.py) runs the identical seeded
        # search twice from the identical starting point -- once with
        # boldness left unbounded (mirrors real production exactly) and
        # once capped at --cap -- and reports how often each arm's best
        # candidate clears the fold-aggregate accepts() gate and the sealed
        # holdout. Read-only, same guarantees as disagreement-sweep:
        # boldness_scan never calls Genome.save()/.promote()/
        # EvolutionRun._record(), so this never touches live_state.json,
        # never promotes anything for real.
        from core import market
        ...
        g0 = acct.genome
        print(f"[boldness-scan] loading {len(g0.universe)}-symbol universe "
              f"({g0.bar_interval} bars) ...", flush=True)
        data = market.load_universe(g0.universe, g0.bar_interval, 4.0)
        if not data:
            print("no market data")
        ...
        print(f"[boldness-scan] champion v{g0.version}, {generations} generation(s), "
              f"cap={cap}, seed={seed}, n_blind={n_blind}, seeded from "
              f"{'live researcher_memory' if seed_live else 'a blank slate'} "
              f"(tested={len(init_tested)}, stagnation={init_stagnation}, "
              f"holdout_draws={init_holdout_draws})")
        print()
        result = boldness_scan(g0, ev, generations=generations, n_blind=n_blind,
                               boldness_cap=cap, seed=seed,
                               initial_tested=init_tested,
                               initial_stagnation=init_stagnation,
                               initial_holdout_draws=init_holdout_draws)
        for label, key in (("UNCAPPED (mirrors real production)", "uncapped"),
                           (f"CAPPED at boldness={cap}", "capped")):
            r = result[key]
            best = [b for b in r["best_fold_fitness_per_gen"] if b is not None]
            print(f"{label}")
            print("=" * 78)
            print(f"  effective boldness per generation: "
                  f"{', '.join(f'{b:.0f}' for b in r['effective_boldness_per_gen'])}")
            print(f"  best-of-generation fold-fitness: "
                  f"{', '.join(f'{b:.3f}' for b in best) if best else '(none proposed)'}")
            print(f"  fold-aggregate gate clears: {r['fold_gate_clears']}")
            print(f"  sealed-holdout gate clears (= shadow promotions): "
                  f"{r['holdout_gate_clears']}")
            print(f"  final stagnation counter: {r['final_stagnation']}")
            print(f"  final in-memory champion version: v{r['final_champion_version']}"
                  f"{' (unchanged)' if r['final_champion_version'] == g0.version else ' (shadow-promoted)'}")
            print()
        print("Reading it: both arms share one seeded Researcher stream at the point "
              "they start, so any difference in gate-clear counts or best-of-generation "
              "fitness is attributable to the boldness cap alone, not RNG luck. Never "
              "touches live_state.json, never promotes anything for real -- "
              "boldness_scan's own read-only contract, same as disagreement_scan.")
    else:

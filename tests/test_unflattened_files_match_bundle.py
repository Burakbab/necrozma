...
from tools.edit_bundle_module import get_module_source, list_modules, module_to_path, pkgs
...
def _bundle_modules():
    ...
    return bundle_text, pkg_list, modules


@pytest.mark.parametrize("module", list_modules(m.BUNDLE_PATH.read_text()))
def test_real_file_matches_embedded_source(module):
    bundle_text, pkg_list, _ = _bundle_modules()
    path = module_to_path(module, pkg_list, REPO_ROOT)
...
def test_no_extra_or_missing_real_package_files():
    ...
    bundle_text, pkg_list, modules = _bundle_modules()
    expected = {str(module_to_path(mod, pkg_list, REPO_ROOT)) for mod in modules}
    actual = set()
    for pkg in pkg_list:
